...
class SearchRequest(BaseModel):
    ...
    capability: Optional[Literal["streaming", "pushNotifications"]] = Field(
        None, description="The capability the agent should support"
    )
    default_input_mode: Optional[List[InputMode]] = Field(
        None, description="The default input mode the agent should support"
    )
    default_output_mode: Optional[List[OutputMode]] = Field(
        None, description="The default output mode the agent should support"
    )
    search_depth: Optional[Literal["advanced", "basic"]] = Field(
        None, description="Controls the latency vs. relevance tradeoff"
    )
...
class AgentDetails(BaseModel):
    ...
    agent_url: Optional[str] = Field(
        None, description="The URL of the agent's card"
    )
    organization_name: Optional[str] = Field(
        None, description="The name of the organization"
    )
    organization_url: Optional[str] = Field(
        None, description="The URL of the organization"
    )
...
class SearchResponse(BaseModel):
    ...
    message: Optional[str] = Field(
        None,
        description="Optional message from the API (e.g. 'Found N agent(s)')",
    )
